...
use App\Http\Requests\Backend\__Main\Broadcaster\Report\StoreRequest;
use App\Http\Requests\Backend\__Main\Broadcaster\Report\UpdateRequest;
...
class ReportController extends Controller {
  ...
  function __construct() {
    ...
    $this->model = 'App\Models\Backend\__Main\Broadcaster\Report';
    $this->path = 'pages.backend.__main.broadcaster.report.';
    $this->url = '/dashboard/broadcasters/reports';
    if (request('date')) { $this->data = $this->model::orderby('date', 'desc')->whereBetween('date', [\Carbon\Carbon::parse(request('date')), \Carbon\Carbon::parse(request('date'))])->get(); }
  ...
  **/
  ...
  public function index() {
    ...
    if (request()->ajax()) {
      ...
      ->editColumn('date', function ($order) { return empty($order->date) ? NULL : \Carbon\Carbon::parse($order->date)->format('Y - F'); })
      ->editColumn('link', function ($order) { return '<a href="' . $order->link . '" target="_blank"><i class="text-primary icon-md fas fa-link"></i></a>'; })
